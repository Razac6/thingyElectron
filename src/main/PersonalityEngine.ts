...
interface PersonalityContext {
  ...
  idleTimeMin?: number;
  avgVelocity?: number; // story points / active day
}
...
export class PersonalityEngine {
  ...
  generateMessage(ctx: PersonalityContext): string {
    const isFirstMessage = this.lastContext === null;
    ...
    if (ctx.mood === 'PANIC') category = 'PANIC';
      ...
      else if (ctx.focusScore < 50 && ctx.habitScore && ctx.habitScore < 0.3)
        category = 'FATIGUE';
    }

    // Nothing dramatic going on and this is the first message of the session - greet instead
    // of falling through to a random tidbit.
    if (category === 'RANDOM' && isFirstMessage) category = 'STARTUP';
    ...
    // 2. Inject Data
    message = message
      .replace('{{userName}}', ctx.userName || 'Użytkowniku')
      .replace('{{streak}}', String(ctx.currentStreak || 0))
      .replace('{{avgVelocity}}', (ctx.avgVelocity || 0).toFixed(1));
