...
export class ProductivityAnalyst {
  ...
  static generateDailyChallenge(
    ...
    if (dailyMode === 'boost') {
      ...
      return Math.random() > 0.5
        ...
        : {
            ...
            description:
              'Boost Mode: Eat the Frog! Complete 1 High Priority task.',
            xpReward: 300,
          };
    }
    ...
    if (trend.direction === 'decreasing') {
      return Math.random() > 0.5
        ? {
            ...
            target: 3, // 3 tasks
            description:
              'Momentum Builder: Complete 3 tasks to get back on track.',
            xpReward: 100,
          }
        : {
            type: 'TASK_SPRINTER',
            ...
            description:
              'Task Sprinter: Complete 3 small tasks (estimate < 1h).',
            xpReward: 120,
          };
    }
    ...
    return {
      ...
      description:
        'Focus Challenge: Accumulate 60 minutes of Deep Work (sessions > 20m).',
      xpReward: 100,
    };
  }
  ...
  static generateDailyTip(
    ...
    if (meetingTime > 180)
      return '⚠️ Meeting Overload (>3h). Your cognitive resources are drained. Stick to low-focus execution tasks today.';
    if (meetingTime > 90)
      return 'Moderate meeting load today. Good for maintenance work, but deep focus might be fragmented.';

    // 1. Check Bio-Data
    if (sleepScore < 50)
      return 'Low sleep detected. Your cognitive function might be reduced. Stick to administrative tasks and take frequent breaks.';
    if (sleepScore > 90)
      return 'Great sleep score! Your brain is primed for learning and complex problem solving today.';

    // 2. Check Daily Mode
    if (dailyMode === 'recovery')
      return 'You are in Recovery Mode. Be kind to yourself. Completing even one small task is a win today.';
    if (dailyMode === 'boost')
      return "Boost Mode Active! Tackle that one big task you've been avoiding. Momentum is on your side.";

    // 3. Check Trend
    if (trend.direction === 'decreasing')
      return 'Your momentum is slowing down. Try a small 5-minute task to get back in the groove.';
    if (trend.direction === 'increasing')
      return 'You are on a roll! Consistency is key. Try to maintain this pace without burning out.';

    // 4. Check Fatigue
    if (fatigue.maxRecommended < 30)
      return 'Your recent sessions suggest quick fatigue. Try the Pomodoro technique (25m work / 5m break) to sustain focus.';
   ...
   */
  static analyzeFocusQuality(sessions: WorkSession[]): {
    score: number;
    ...
    sessions.forEach((session) => {
      ...
      if (durationMinutes >= 20 && durationMinutes <= 120) {
        const endTime =
          new Date(session.startTime).getTime() + session.duration;
   ...
   */
  static identifyPeakHours(sessions: WorkSession[]): {
    peakHours: number[];
    ...
    if (topHours.length > 0) {
      ...
      const endHour = (topHours[topHours.length - 1] + 1) % 24;
      range = `${topHours[0]}:00 - ${endHour}:00`;
    }
   ...
   */
  static analyzeFatigue(
    sessions: WorkSession[],
    currentSessionMinutes?: number,
  ): { averageSession: number; maxRecommended: number; isFatigued: boolean } {
    if (sessions.length < 5) {
      ...
      return {
        averageSession: avg,
        maxRecommended: max,
        isFatigued: currentSessionMinutes ? currentSessionMinutes > max : false,
      };
    }
    ...
    if (validDurations.length === 0)
      return { averageSession: 45, maxRecommended: 60, isFatigued: false };
    ...
    const variance =
      validDurations.reduce((a, b) => a + (b - mean) ** 2, 0) /
    ...
    return {
      ...
      isFatigued: currentSessionMinutes
        ? currentSessionMinutes > maxRecommended
        : false,
    };
  }
   ...
   */
  static analyzeTrend(dailyTotals: { date: string; totalDuration: number }[]): {
    slope: number;
    direction: 'increasing' | 'decreasing' | 'stable';
    description: string;
  } {
    ...
    const sortedData = [...dailyTotals].sort((a, b) =>
    ...
    if (slope > 2) {
      ...
      description =
        'Masz świetne momentum! Twoja produktywność rośnie z dnia na dzień.';
    } else if (slope < -2) {
   ...
   * High CV = Volatile.
   */
  static analyzeTagConsistency(
    tagAnalytics: any[],
    tagsMap: Map<number, string>,
  ): { consistent: string[]; volatile: string[] } {
    ...
    tagAnalytics.forEach((stat) => {
      ...
      // Benchmarks:
      // CV < 0.25 (25% variance) is very consistent
      // CV > 0.6 (60% variance) is highly volatile
      if (cv < 0.25) {
  ...
  static analyzeTagDifficulty(tasks: any[]): Record<string, number> {
    ...
    Object.entries(tagStats).forEach(([tag, stats]) => {
      // Only calculate if we have significant data (> 1 hour of total estimated work)
      if (stats.totalEst > 60 * 60 * 1000) {
        difficultyMap[tag] = Number(
          (stats.totalSpend / stats.totalEst).toFixed(2),
        );
    ...
    difficultyMap.default =
      globalEst > 0 ? Number((globalSpend / globalEst).toFixed(2)) : 1.0;
   ...
   */
  static suggestBestTimeForTask(
    task: any,
    peakHours: number[],
  ): { icon: string; text: string; color: string } {
    const isHard =
      task.priority === 'High' || (task.estimate && task.estimate >= 2);
    const isEasy =
      task.priority === 'Low' || (task.estimate && task.estimate < 1);
    ...
    if (isHard) {
      if (peakHours.length > 0) {
        ...
        return {
          icon: '⚡',
          text: `Golden Hour (${start}:00-${end}:00)`,
          color: 'warning',
        };
      }
  ...
  static calculateTaskScore(
    task: any,
    tagDifficulty: Record<string, number>,
    sprintEndDate?: string,
    neuralEstimateHours?: number,
  ): number {
    let score = 0;
   ...
   */
  static analyzeHabitCorrelation(
    dailyHabitScores: { date: string; score: number }[],
    sessions: WorkSession[],
    ...
    dailyHabitScores.forEach((hs) => {
      ...
      const deepWork = durations
        .filter((d) => d >= 20 * 60 * 1000 && d <= 120 * 60 * 1000)
        .reduce((a, b) => a + b, 0);
    ...
    if (highHabitDays.length >= 2 && lowHabitDays.length >= 2) {
      const avgHigh =
        highHabitDays.reduce((a, b) => a + b, 0) / highHabitDays.length;
      const avgLow =
        lowHabitDays.reduce((a, b) => a + b, 0) / lowHabitDays.length;
  ...
  static analyzeSprintRisk(
    ...
    if (totalTasks === 0)
      return {
        ...
        total: 0,
        risk: 'Stable',
        message: 'Brak zadań w sprincie.',
      };

    // 1. Calculate remaining workload (Hours)
    const remainingMinutesAI = neuralPredictions.reduce(
      (acc, val) => acc + val,
      0,
    );
    const remainingHours =
      remainingMinutesAI > 0
        ? remainingMinutesAI / 60
        : unfinishedTasks.reduce((acc, t) => acc + (t.estimate || 1), 0);
    ...
    let totalCapacity = hoursLeftToday + (daysLeft - 1) * dailyWorkHours;
    ...
    // If the workHours-based capacity is unrealistic (e.g. misconfigured work hours),
    // fall back to capacity derived from the user's actual historical daily velocity.
    if (!Number.isFinite(totalCapacity) || totalCapacity <= 0) {
      totalCapacity = Math.max(daysLeft, 1) * avgVelocity;
    }
