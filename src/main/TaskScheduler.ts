...
export const getProposedSchedule = (userId: number) => {
  ...
  const scoredTasks = actionableTasks.map((task: any) => {
    ...
    const score = ProductivityAnalyst.calculateTaskScore(
      task,
      difficultyProfile,
      endDate,
      neuralEstimateHours,
    ...
    else if (neuralEstimateHours < (task.estimate || 0))
      reasons.push('📉 Efficiency Opportunity');
    ...
    return {
      ...
      score,
      aiReason: reason,
      neuralEstimate: neuralEstimateHours,
    };
  });
...
export const autoScheduleTasks = (userId: number) => {
  ...
  log.info(
    `[TaskScheduler] Auto-scheduled ${ids.length} tasks for user ${userId}.`,
  );
  return sortedTasks;
};
