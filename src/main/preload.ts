...
contextBridge.exposeInMainWorld('electron', {
  database: {
    ...
    getWorkSessions: (userId: number, days: number) => ipcRenderer.invoke('db:get-work-sessions', userId, days),
    getTaskWorkSessions: (taskId: number) => ipcRenderer.invoke('db:get-task-work-sessions', taskId),
    getLast14DaysProductivity: (userId: number) => ipcRenderer.invoke('db:get-last-14-days-productivity', userId),
    getDeepWorkHistory: (userId: number, days: number) => ipcRenderer.invoke('db:get-deep-work-history', userId, days),
    ...
    getDistractionStats: (days: number) => ipcRenderer.invoke('db:get-distraction-stats', days),
