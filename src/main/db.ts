...
export const getNeuralConfidence = () => {
  ...
  try {
    ...
    const taskCountStmt = db.prepare(
      "SELECT COUNT(*) as count FROM tasks WHERE status = 'Completed' COLLATE NOCASE",
    );
    ...
    const tagStmt = db.prepare(
      'SELECT COUNT(*) as count FROM tag_analytics WHERE completed_count >= 3',
    );
    ...
    const sprintStmt = db.prepare(
      "SELECT COUNT(*) as count FROM sprints WHERE status = 'COMPLETED' COLLATE NOCASE",
    );
    ...
    const habitStmt = db.prepare(
      'SELECT COUNT(*) as count FROM habit_logs WHERE value >= 1',
    );
...
export const getAiMaturity = () => {
  ...
  const taskCountStmt = db.prepare(
    "SELECT COUNT(*) as count FROM tasks WHERE status = 'Completed' AND spendTime > 0",
  );
  const taskRow = taskCountStmt.getAsObject();
...
export const getAiStats = () => {
  if (!db)
    return { maturity: 0, confidence: 0, trainingCount: 0, dataCount: 0 };
...
export const createTask = (task: any, userId: number) => {
  ...
  const stmt = db.prepare(
    `INSERT INTO tasks (title, description, status, updateStatusDate, estimate, priority, link, createdAt, spendTime, startTimer, type, userId, sprintId, displayOrder) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  );
  stmt.run([
    newTask.title,
    newTask.description,
...
export const updateTask = (task: any) => {
  ...
  db.run(
    `UPDATE tasks SET title = ?, description = ?, status = ?, updateStatusDate = ?, estimate = ?, priority = ?, link = ?, spendTime = ?, startTimer = ?, sprintId = ?, type = ? WHERE id = ?`,
    [
      task.title,
      task.description,
      task.status,
...
export const createSprint = (sprint: {
  ...
  if (!db) throw new Error('DB not initialized');
  db.run(
    'INSERT INTO sprints (name, startDate, endDate, status) VALUES (?, ?, ?, ?)',
    [sprint.name, sprint.startDate, sprint.endDate, 'UPCOMING'],
  );
  const id = db.exec('SELECT last_insert_rowid() as id')[0].values[0][0];
  saveDB();
...
export const updateSprint = (sprint: any) => {
  if (!db) throw new Error('DB not initialized');
  db.run(
    'UPDATE sprints SET name = ?, startDate = ?, endDate = ?, status = ? WHERE id = ?',
...
export const getDailyBio = (date: string) => {
  ...
  if (!result || !result.date)
    return { mode: 'normal', sleepScore: null, meetingTime: 30 }; // Default 30 min
  return {
...
export const updateDailyBio = (
  date: string,
  data: { mode?: string; sleepScore?: number; meetingTime?: number },
) => {
  if (!db) throw new Error('DB not initialized');
  ...
  db.run(
    'INSERT OR REPLACE INTO daily_energy_logs (date, mode, sleepScore, meetingTime) VALUES (?, ?, ?, ?)',
    [date, newMode, newSleep, newMeetingTime],
  );
  saveDB();
...
export const getDailyStandupData = (userId: number) => {
  ...
  const lastActiveDayStmt = db.prepare(
    "SELECT updateStatusDate FROM tasks WHERE userId = ? AND status = 'Completed' ORDER BY updateStatusDate DESC LIMIT 1",
  );
  ...
  if (lastActiveResult && lastActiveResult.updateStatusDate) {
    const rawDate = lastActiveResult.updateStatusDate;
    // Handle ISO format YYYY-MM-DDTHH:mm:ss.sssZ
    if (rawDate.includes('T')) {
      lastActiveDateStr = rawDate.split('T')[0];
    } else {
      // Handle 'YYYY-MM-DD HH:mm:ss' or 'YYYY-MM-DD'
      lastActiveDateStr = rawDate.split(' ')[0];
    }

            // Handle 'DD.MM.YYYY' legacy format

            if (lastActiveDateStr.includes('.') && lastActiveDateStr.split('.').length === 3) {

                const [day, month, year] = lastActiveDateStr.split('.');

                if (year.length === 4) {

                    lastActiveDateStr = `${year}-${month}-${day}`;
  ...
  const lastDayStatsStmt = db.prepare(
    "SELECT COUNT(*) as count, SUM(spendTime) as totalTime FROM tasks WHERE userId = ? AND status = 'Completed' AND updateStatusDate LIKE ?",
  );
  const lastDayStats = lastDayStatsStmt.getAsObject([
    userId,
    `${lastActiveDateStr}%`,
  ]);
  ...
  // 3. Tasks for today
  const remainingTasks = getTasks(userId).filter(
  ...
  return {
    ...
    yesterday: {
      // Keeping key 'yesterday' for frontend compat, but content is 'last active'
