...
function HabitHistoryChart({
  ...
  const data = useMemo(() => {
    ...
    if (range === 'all' && logs.length > 0) {
      ...
      daysToScroll = Math.min(
        Math.ceil(
          (new Date().getTime() - earliest.getTime()) / (1000 * 60 * 60 * 24),
        ) + 1,
        365,
      );
    }
...
function HabitCard({
  ...
  const isCompleted = (dateStr: string) =>
    logs.some((l) => l.date === dateStr && l.value >= 1);

  return (
    <Card
      className="habit-card"
      sx={{ mb: 2, borderRadius: 3, overflow: 'visible' }}
    >
      <CardContent
        sx={{ p: '24px !important', cursor: 'pointer' }}
      ...
      >
        <Grid container alignItems="center" spacing={2}>
              ...
              <Box>
                ...
                <Box display="flex" alignItems="center" gap={1} mt={0.5}>
                  <Chip
                    ...
                    size="small"
                    sx={{ fontSize: '0.65rem', height: 20, bgcolor: '#f0f0f0' }}
                  />
                  <Box
                    display="flex"
                    alignItems="center"
          ...
          <Grid item xs={12} md={5}>
            <Box
              className="week-bubbles-container"
              ...
              {weekDates.map((date) => {
                ...
                return (
                  ...
                  >
                    <Typography
                      variant="caption"
                      className={`day-label ${isToday ? 'today' : ''}`}
                    >
                      {DAYS_OF_WEEK.find(
                        (d) => d.value === date.getDay(),
                      )?.label.charAt(0)}
                    </Typography>
                    <Box
                      className={`day-bubble ${done ? 'done' : ''} ${isToday ? 'today-bubble' : ''} ${isPendingToday ? 'pending-today' : ''}`}
                    >
                      {done && (
          ...
          >
            <IconButton
              ...
              sx={{ color: habit.isFavorite ? '#ffb703' : '#9e9e9e' }}
            >
              {habit.isFavorite ? <StarIcon /> : <StarBorderIcon />}
            </IconButton>
            <IconButton size="small" onClick={onEdit} sx={{ color: '#9e9e9e' }}>
  ...
  const handleSave = async () => {
    try {
      if (isEditing && currentHabit.id) {
        await window.electron.database.updateHabit({
          ...currentHabit,
          id: currentHabit.id,
        });
      } else {
                ...
                >
                  {DAYS_OF_WEEK.map((day) => (
                    <MenuItem key={day.value} value={day.value}>
                      <Checkbox
                        checked={
                          currentHabit.frequency!.days.indexOf(day.value) > -1
                        }
                      />
