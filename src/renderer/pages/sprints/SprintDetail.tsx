...
export default function SprintDetail() {
  ...
  useEffect(() => {
    const fetchSprintData = async () => {
      ...
      const avgCapacity =
        await window.electron.database.getAverageSprintCapacity();
  ...
  useEffect(() => {
    let cancelled = false;
    analyzeSprintOptimism(tasksInSprint).then((warning) => {
      if (!cancelled) setOptimismWarning(warning);
    });
    return () => {
      cancelled = true;
    };
  ...
  const capacityProgress =
    calculatedCapacity > 0
      ? Math.min((currentLoad / calculatedCapacity) * 100, 100)
      : 0;

  const velocityDiff =
    historicalVelocity > 0 ? currentLoad - historicalVelocity : 0;
  ...
  if (!sprint)
    return <Typography sx={{ p: 3 }}>Loading sprint data...</Typography>;
  ...
  return (
    ...
    >
      ...
      <Grid
        container
        spacing={3}
        sx={{ px: 3, pb: 3, flex: 1, overflow: 'hidden' }}
      >
        {/* Left Column: Tasks */}
        <Grid
          item
          ...
          >
            ...
            >
              <Typography variant="subtitle1" fontWeight="bold">
                Sprint Backlog ({tasksInSprint.length})
              </Typography>
              <Button
                variant="contained"
                size="small"
                ...
                tasksInSprint.map((task) => (
                  ...
                  >
                    ...
                    <ListItemText
                      ...
                      secondary={
                        <Typography variant="caption" color="text.secondary">
                          {task.status} • Est: <strong>{task.estimate}h</strong>{' '}
        ...
        <Grid item xs={12} md={4} sx={{ height: '100%', overflow: 'auto' }}>
            ...
            <Box mt={2}>
              <Box display="flex" justifyContent="space-between" mb={0.5}>
                ...
                <Typography variant="body2" fontWeight="bold">
                  {currentLoad}h{' '}
                  <span style={{ color: '#bdbdbd', fontWeight: 'normal' }}>
                    / {calculatedCapacity}h
                  </span>
                </Typography>
              </Box>
              <LinearProgress
          ...
          <Paper sx={{ p: 3, mb: 2 }}>
            ...
            <Box display="flex" alignItems="center" gap={2} mt={2}>
              <SpeedIcon
                fontSize="large"
                color={isOverVelocity ? 'warning' : 'success'}
              />
              <Box>
            ...
            {isOverVelocity && (
              <Alert severity="warning" sx={{ mt: 2, py: 0 }}>
                You are planning <strong>{velocityDiff.toFixed(1)}h</strong>{' '}
                more than usual.
              </Alert>
            )}
            {optimismWarning && (
          ...
          {/* 3. AI Risk Assessment Widget - only fetched for active sprints */}
          {aiAnalysis && (
            <Paper sx={{ p: 3, mb: 2 }}>
              <Typography
                variant="overline"
              ...
              <Box display="flex" alignItems="center" gap={2} mt={2}>
                <FlagIcon
                  ...
                  sx={{
                    color:
                      aiAnalysis.risk === 'Critical'
                        ? '#d32f2f'
                        : aiAnalysis.risk === 'At Risk'
                          ? '#fb8500'
                          : '#2e7d32',
                  }}
                />
                <Box>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    {aiAnalysis.completed}/{aiAnalysis.total} tasks completed
                  </Typography>
                </Box>
              </Box>
              {aiAnalysis.message && (
          ...
          <List sx={{ mt: 2, maxHeight: 400, overflow: 'auto' }}>
            {backlogTasks.map((task) => (
              ...
              >
                <ListItemText
                  primary={task.title}
                  secondary={`Estimate: ${task.estimate}h`}
                />
              </ListItemButton>
            ))}
          </List>
