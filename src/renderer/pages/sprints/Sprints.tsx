...
function Sprints() {
  ...
  const [editingSprint, setEditingSprint] = useState<SprintInterface | null>(
    null,
  );
    ...
    setSprints(
      sprintsData.map((s) => ({
        ...
        excludedDates: s.excludedDates
          ? s.excludedDates.map((d: any) => new Date(d))
          : [],
      })),
    );
  };
  ...
  const finalCapacity =
    manualCapacity !== null ? manualCapacity : calculatedCapacity;
  ...
  const handleOpenForm = (sprint?: SprintInterface) => {
    if (sprint) {
      ...
      const excluded = sprint.excludedDates
        ? Array.isArray(sprint.excludedDates) &&
          typeof sprint.excludedDates[0] === 'string'
          ? sprint.excludedDates.map((d: any) => new Date(d))
          : (sprint.excludedDates as unknown as Date[])
        : [];
      setExcludedDates(excluded || []);
      setManualCapacity(
        sprint.capacity && sprint.capacity !== calculatedCapacity
          ? sprint.capacity
          : null,
      );
    } else {
        ...
        if (params.row.id === editingStatusId) {
          return (
            <Select
              value={params.value}
              onChange={(e) =>
                handleUpdateSprintStatus(params.row.id, e.target.value as any)
              }
  ...
  return (
    ...
    >
      ...
      <Typography
        variant="h4"
        gutterBottom
        sx={{ fontWeight: 300, flexShrink: 0 }}
      >
        Sprint Management
      </Typography>
