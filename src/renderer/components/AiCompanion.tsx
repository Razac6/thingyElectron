...
export function AiCompanion() {
  ...
  const [menuView, setMenuView] = useState<
    'main' | 'distractions' | 'report' | 'suggest' | 'standup'
  >('main');
  ...
  const [promptType, setPromptType] = useState<
    'none' | 'water' | 'meditation' | 'stretching'
  >('none');
  ...
  useEffect(() => {
    ...
    const handleShowMessage = (msg: any) => {
      ...
      if (
        lowMsg.includes('nawodnieniu') ||
        lowMsg.includes('wodzie') ||
        lowMsg.includes('wodę') ||
        lowMsg.includes('wody')
      )
        setPromptType('water');
      else if (
        lowMsg.includes('ruch') ||
        lowMsg.includes('wyprostuj') ||
        lowMsg.includes('szyję') ||
        lowMsg.includes('ćwiczenia')
      )
        setPromptType('stretching');
      else if (
        lowMsg.includes('mindfulness') ||
        lowMsg.includes('oddech') ||
        lowMsg.includes('medytac')
      )
        setPromptType('meditation');
  ...
  useEffect(() => {
    ...
    const scheduleNextAppearance = () => {
      const nextTime =
        Math.random() * (45 * 60 * 1000 - 15 * 60 * 1000) + 15 * 60 * 1000;
      return setTimeout(() => {
  ...
  const triggerAppearance = async (forceMessage = false) => {
    ...
    let msg = null;
    const type: 'none' | 'water' | 'meditation' | 'stretching' = 'none';
  ...
  const handleLogActivity = async (
    activity: 'water' | 'meditation' | 'stretching',
  ) => {
    const today = new Date().toISOString().split('T')[0];
    try {
      if (activity === 'water') {
        ...
        // @ts-ignore
        await window.electron.database.updateDailyBio(today, {
          waterIntake: newVal,
        });
        setMessage('Super! Tak trzymaj. 🌊');
        if (await checkForAchievements('HEALTH_ACTION'))
  ...
  const handleSnooze = (minutes: number) => {
    ...
    setTimeout(
      () => {
        const msg =
          promptType === 'meditation' ? '🧘‍♀️ Gotowy na medytację?' : 'Gotowy?';
        setIsVisible(true);
        setMessage(msg);
    ...
    switch (action) {
      case 'report':
        ...
        try {
          // @ts-ignore
          const data =
            await window.electron.database.getDailyReportData(userId);
          setReportData(data);
          setMessage('Twoje dzisiejsze wyniki:');
        } catch (e) {
          setMessage('Błąd pobierania raportu.');
        }
        break;
      case 'standup_init':
        setIsVisible(true);
        ...
        try {
          ...
          setReportData({
            yesterday: data.yesterday || { completedCount: 0, totalTimeMs: 0 },
          });
          setSuggestion({
  ...
  const bubbleWidth =
    menuView === 'standup' ? 360 : isMenuOpen ? 260 : 'max-content';
      ...
      <Fade in={isVisible && !!message} timeout={500}>
        <Paper
          ...
          sx={{
            ...
            width: bubbleWidth,
            maxWidth: menuView === 'standup' ? 400 : 280,
            pointerEvents: 'auto',
        ...
        >
          <Typography
            ...
            sx={{
              ...
              color: '#023047',
              display: 'block',
              mb: isMenuOpen || promptType !== 'none' ? 1.5 : 0,
            }}
          >
            {renderTextWithIcons(message)}
          </Typography>
          ...
          {promptType !== 'none' && !isMenuOpen && !showSnoozeOptions && (
            <Stack direction="row" spacing={1}>
              <Button
                ...
                variant="contained"
                color="primary"
                onClick={() => handleLogActivity(promptType as any)}
                sx={{ fontSize: '0.7rem' }}
              >
          ...
          {isMenuOpen && menuView === 'main' && (
            <Stack spacing={1}>
              {settings.enable_water_reminders === 'true' && (
                ...
                >
                  ...
                  <Box display="flex" alignItems="center" gap={1}>
                    ...
                    >
                      {waterIntake} <LocalDrinkIcon fontSize="inherit" />
                    </Typography>
                    <Button
                      size="small"
                      sx={{ minWidth: 30, p: 0, height: 20 }}
                      onClick={addWater}
                    >
                      +
                    </Button>
              ...
              <Button
                ...
                onClick={() => handleAction('standup_init')}
                sx={{
                  justifyContent: 'flex-start',
                  textTransform: 'none',
                  color: '#023047',
          ...
          {isMenuOpen && menuView === 'standup' && reportData && (
            <Stack spacing={2} sx={{ mt: 1 }}>
              <Box>
                ...
                <Box display="flex" justifyContent="space-between">
                  ...
                  <Typography variant="caption" fontWeight="bold">
                    {reportData.yesterday?.completedCount || 0}
                  </Typography>
                </Box>
                <Box display="flex" justifyContent="space-between">
                  <Typography variant="caption">Czas:</Typography>
                  <Typography variant="caption" fontWeight="bold">
                    {formatDuration(reportData.yesterday?.totalTimeMs || 0)}
                  </Typography>
                </Box>
              </Box>
              <Divider />
              <Box>
                ...
                >
                  DZISIEJSZE WYZWANIE <TrackChangesIcon fontSize="inherit" />
                </Typography>
                <Typography
                  variant="body2"
                  sx={{ fontSize: '0.75rem', fontWeight: 300 }}
                >
                  {suggestion?.challenge?.description || 'Brak wyzwania.'}
                </Typography>
              </Box>
              <Divider />
              <Box>
                ...
                >
                  SUGEROWANY START <RocketLaunchIcon fontSize="inherit" />
                </Typography>
                {suggestion?.topSuggestion ? (
                  <Box bgcolor="rgba(0,0,0,0.03)" p={1} borderRadius={1}>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        navigate(`/task/${suggestion.topSuggestion.id}`);
                        closeEverything();
                      }}
                    >
                      Otwórz
              ...
              <Button
                ...
                sx={{ textTransform: 'none', fontSize: '0.7rem', opacity: 0.7 }}
              >
                Wróć
              </Button>
            </Stack>
            ...
            <Stack spacing={1}>
              {distractions.map((d, i) => (
                <Box
                  ...
                  justifyContent="space-between"
                  alignItems="center"
                  sx={{ fontSize: '0.75rem', color: '#023047' }}
                >
          ...
          {isMenuOpen && menuView === 'report' && reportData && (
            <Stack spacing={1} sx={{ mt: 1 }}>
              ...
              <Box display="flex" justifyContent="space-between">
                <Typography variant="caption">Czas:</Typography>
                <Typography variant="caption" fontWeight="bold">
                  {formatDuration(reportData.totalTimeMs || 0)}
                </Typography>
              </Box>
              <Box display="flex" justifyContent="space-between">
                <Typography variant="caption">Pomodoro:</Typography>
                <Typography
                  variant="caption"
                  fontWeight="bold"
                  sx={{ display: 'flex', alignItems: 'center', gap: 0.25 }}
