  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        try {
          // @ts-ignore
          const perfData = await window.electron.database.getAiPerformance(
            userId,
            7,
          );
          ...
          if (!perfData || perfData.length === 0) {
            ...
            const totalActual = perfData.reduce(
              (acc: number, d: any) => acc + (d.actual || 0),
              0,
            );
  ...
  const tooltipText = (
    <>
      <Typography variant="body2" color="inherit">
        Ten wykres to Twoje "Lustro Produktywności". Porównuje Twój rzeczywisty
        czas pracy z tym, co przewiduje AI.
      </Typography>
      <Typography variant="body2" color="inherit" mt={1} component="div">
        <ul>
          <li>
            <strong>Rzeczywistość &lt; AI:</strong> Jesteś w stanie Flow.
          </li>
          <li>
            <strong>Rzeczywistość &gt; AI:</strong> Coś Cię blokuje lub zadanie
            jest trudniejsze niż myślałeś.
          </li>
        </ul>
      </Typography>
    </>
    ...
    <Paper sx={{ p: 2, position: 'relative', overflow: 'hidden' }}>
      ...
      <Box sx={{ height: 300, position: 'relative' }}>
        {!hasData && (
          ...
          >
            <Alert severity="info">
              Not enough data yet. Complete some tasks to see AI comparison.
            </Alert>
          </Box>
        )}
        {chartData && (
