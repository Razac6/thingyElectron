...
import { toLocalISOString, addDays, getMonday } from '../util/dateUtils';
...
export default function FavoriteHabitWidget() {
    ...
    try {
      ...
      if (topHabit) {
        ...
        const completedDates = [
          ...new Set(
            habitLogs.filter((l: any) => l.value >= 1).map((l: any) => l.date),
          ),
        ];
  ...
  const weekDates = Array.from({ length: 7 }, (_, i) =>
    toLocalISOString(addDays(monday, i)),
  );
      ...
      >
        <Box display="flex" alignItems="center" gap={1}>
          <Typography
            variant="caption"
            fontWeight="bold"
            sx={{ color: '#023047', fontSize: '0.75rem' }}
          >
            {habit.title}
          </Typography>
        ...
        <Box display="flex" gap={0.8}>
          {weekDates.map((dateStr) => {
            ...
            return (
              <Tooltip
                key={dateStr}
                title={isToday ? 'Click to toggle today' : dateStr}
                arrow
              >
                <Bubble
