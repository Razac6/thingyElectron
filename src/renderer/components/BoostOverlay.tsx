...
function BoostOverlay({ open, onClose: _onClose }: BoostOverlayProps) {
  ...
  const refreshChecklist = async () => {
    ...
    try {
      const items = await window.electron.database.getChecklistItems(
        activeTask.id,
      );
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (isPomodoro) {
        ...
        const p = (elapsed / estimateMs) * 100;
        setProgress(p);
  ...
  const handlePomodoroComplete = async () => {
    console.log(
      'BoostOverlay: Pomodoro Timer Finished! Triggering completePomodoro...',
    );
    if (!activeTask) return;
  ...
  const handleAddSubtask = async () => {
    ...
    try {
      await window.electron.database.addChecklistItem(
        activeTask.id,
        newSubtaskTitle,
      );
      setNewSubtaskTitle('');
  ...
  const handleToggleSubtask = async (
    ...
    try {
      await window.electron.database.toggleChecklistItem(
        itemId,
        !currentStatus,
      );
      refreshChecklist();
  ...
  const waveColorLight = isPomodoro
    ? '#e57373'
    : isOvertime
      ? '#e57373'
      : '#64b5f6';
      ...
      >
        <Fab
          ...
          sx={{
            ...
            top: 32,
            right: 32,
            bgcolor: isPomodoro ? '#ef5350' : 'primary.main',
          }}
        ...
        >
          ...
          >
            ...
            <Typography
              ...
              sx={{
                opacity: 0.9,
                color: isOvertime || isPomodoro ? '#ffcdd2' : 'inherit',
              }}
            >
                ...
                >
                  <ListItemIcon>
                    <Checkbox
                      ...
                      onChange={() =>
                        handleToggleSubtask(item.id, item.isCompleted === 1)
                      }
                      tabIndex={-1}
                      disableRipple
                    />
                  ...
                  <ListItemText
                    ...
                    sx={{
                      textDecoration:
                        item.isCompleted === 1 ? 'line-through' : 'none',
                      color:
                        item.isCompleted === 1
                          ? 'text.disabled'
                          : 'text.primary',
