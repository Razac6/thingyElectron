...
const ShutdownModal: React.FC<ShutdownModalProps> = ({ open, onClose }) => {
  ...
  const completedCount = tasks.filter(
    (t) =>
      t.status === 'Completed' &&
      t.updateStatusDate === new Date().toLocaleDateString(),
  ).length;
          ...
          <Grid container spacing={2}>
            <Grid item xs={6}>
              ...
              <Typography variant="h6">
                {Math.floor(totalSpendTimeToday / 3600000)}h{' '}
                {Math.floor((totalSpendTimeToday % 3600000) / 60000)}m
              </Typography>
            </Grid>
            <Grid item xs={6}>
              <Typography variant="caption" display="block">
            ...
            <Grid item xs={6}>
              ...
              >
                {dailyChallenge?.status === 'COMPLETED' ? (
                  <>
                    <CheckCircleIcon fontSize="inherit" /> Zrobione
                  </>
                ) : (
                  <>
                    <HourglassEmptyIcon fontSize="inherit" /> W toku
                  </>
                )}
              </Typography>
            </Grid>
