...
function SmartInsightWidget() {
  ...
  const [dailyBio, setDailyBioState] = useState<{
    ...
    meetingTime: number;
  }>({ mode: 'normal', sleepScore: null, meetingTime: 0 });
  const [anchorEl, setAnchorEl] = useState<HTMLButtonElement | null>(null);
  const [meetingAnchorEl, setMeetingAnchorEl] =
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener('open-shutdown-modal', handleOpenShutdown);
  }, []);
  ...
  const { peakHourRange, fatigueProfile, trend, focusScore, habitCorrelation } =
    insights;
  ...
  const focusText =
    typeof focusScore === 'object'
      ? `${focusScore.score}% (${formatDuration(focusScore.deepWorkMinutes || 0)})`
      : `${focusScore}%`;

  return (
    <Box
      sx={{ mt: 2, p: 1, backgroundColor: 'rgba(0,0,0,0.03)', borderRadius: 2 }}
    >
      <Typography
        variant="overline"
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start' }}>
        ...
        <Stack spacing={0.5} sx={{ flexGrow: 1 }}>
          ...
          <Box display="flex" alignItems="center" gap={1}>
            ...
            <Typography variant="caption">
              <strong>Deep Work:</strong> {focusText}
            </Typography>
          </Box>
          ...
          <Box display="flex" alignItems="center" gap={1}>
            ...
            <Typography variant="caption">
              ...
              <span style={{ opacity: 0.6, marginLeft: 4 }}>
                / {fatigueProfile.maxRecommended}m limit
              </span>
            </Typography>
          </Box>
            ...
            <Tooltip title={sprintAnalysis.message} arrow>
              <Box display="flex" alignItems="center" gap={1}>
                <FlagIcon
                  sx={{
                    color:
                      sprintAnalysis.risk === 'Critical'
                        ? '#d32f2f'
                        : sprintAnalysis.risk === 'At Risk'
                          ? '#fb8500'
                          : '#2e7d32',
                    fontSize: 16,
                  }}
          ...
          {habitCorrelation && (
            <Tooltip title={habitCorrelation.message} arrow>
              <Box
                display="flex"
                alignItems="center"
                gap={1}
                sx={{ cursor: 'help' }}
              >
                <EmojiEventsIcon sx={{ color: '#2e7d32', fontSize: 16 }} />
                <Typography variant="caption">
                  <strong>Nawyki:</strong> +{habitCorrelation.impact}% focus
                </Typography>
              </Box>
            </Tooltip>
          )}
          ...
          <ButtonGroup variant="text" size="small" orientation="vertical">
            <Tooltip title="Boost Mode" placement="left">
              <IconButton
                ...
                color={dailyBio.mode === 'boost' ? 'error' : 'default'}
              >
            ...
            <Tooltip title="Normal Mode" placement="left">
              <IconButton
                ...
                color={dailyBio.mode === 'normal' ? 'primary' : 'default'}
              >
            ...
            <Tooltip title="Recovery Mode" placement="left">
              <IconButton
                ...
                color={dailyBio.mode === 'recovery' ? 'success' : 'default'}
              >
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0 }}>
            {settings.enableSleepTracking === 'true' && (
              <IconButton
                ...
                color={dailyBio.sleepScore ? 'primary' : 'default'}
              >
            ...
            <IconButton
              ...
              color={dailyBio.meetingTime > 0 ? 'primary' : 'default'}
            >
            ...
            <Tooltip
              title="Zakończ dzień (Shutdown Ritual)"
              arrow
              placement="left"
            >
        ...
        <Box sx={{ p: 2, width: 220 }}>
          ...
          <Box display="flex" gap={1} mt={1}>
            ...
            <TextField
              ...
              onChange={(e) =>
                handleMeetingUpdate(
                  Math.floor((dailyBio.meetingTime || 0) / 60) * 60 +
