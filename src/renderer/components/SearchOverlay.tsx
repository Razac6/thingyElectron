...
export default function SearchOverlay({ open, onClose }: SearchOverlayProps) {
  ...
  const [showSuggestions, setShowSuggestions] = useState<'TYPE' | 'TAG' | null>(
    null,
  );
    ...
    const fetchAiEstimate = async () => {
      ...
      try {
        const userId = localStorage.getItem('userId')
          ...
          : 1;
        const predMin = await window.electron.database.predictDuration({
          title,
          tags,
          userId,
          priority: 'Medium',
  ...
  useEffect(() => {
    ...
    if (currentWord.startsWith('@')) {
      ...
      // Once the typed word is already a complete, exact match, close the dropdown so
      // Enter submits the task instead of re-applying the same suggestion.
      const isExactMatch = filtered.length === 1 && filtered[0] === filter;
      ...
      const filtered = availableTags.filter((t) =>
        t.toLowerCase().startsWith(filter),
      );
      ...
      // Enter submits the task instead of re-applying the same suggestion.
      const isExactMatch =
        filtered.length === 1 && filtered[0].toLowerCase() === filter;
      if (filtered.length > 0 && !isExactMatch) {
        setSuggestionList(filtered);
      ...
      if (matches) {
        ...
        if (tag && tag.id) {
          ...
          if (analytics && analytics.completed_count > 0) {
            ...
            const stdDevHours = (analytics.std_dev / (1000 * 60 * 60)).toFixed(
              1,
            );
            setSuggestion(
              `Avg time for #${lastTag}: ~${hours}h (± ${stdDevHours}h)`,
            );
          } else {
            setSuggestion(
              `No analytics for #${lastTag} yet. Complete some tasks with this tag!`,
            );
          }
        } else {
          setSuggestion(null);
  ...
  const handleQuickAdd = async () => {
    ...
    const newTask: Partial<Task> = {
      ...
      startTimer: null,
      type,
      tags, // Pass extracted tags to DB
    };
  ...
  const handleKeyDown = (event: React.KeyboardEvent) => {
    if (showSuggestions && suggestionList.length > 0) {
      ...
      if (event.key === 'ArrowUp') {
        event.preventDefault();
        setSuggestionIndex(
          (prev) => (prev - 1 + suggestionList.length) % suggestionList.length,
        );
        return;
      }
            ...
            endAdornment: (
              <InputAdornment position="end">
                <IconButton
                  ...
                  title={
                    isAdding ? 'Switch to Search Mode' : 'Switch to Add Mode'
                  }
                >
            ...
            <List dense>
              {suggestionList.map((item, index) => (
                ...
                >
                  <ListItemText
                    primary={
                      showSuggestions === 'TYPE' ? `@${item}` : `#${item}`
                    }
                  />
                  {index === suggestionIndex && (
                    ...
                    >
                      <KeyboardReturnIcon fontSize="inherit" /> Enter
                    </Typography>
                  )}
                </ListItemButton>
              ))}
